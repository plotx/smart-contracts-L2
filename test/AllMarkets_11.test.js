...
const AllMarkets = artifacts.require("AllPlotMarkets_11");
    ...
    contract("AllMarkets", async function ([user0, user1, user2, user3, user4, user5, userMarketCreator, user6]) {
        ...
        it("1.0 Set user levels", async () => {
          ...
          for(let i = 1; i <= 25; i++) {
              ...
              if(i == 22) {
                feeDiscounts.push(10000);
              } else {
                  feeDiscounts.push(0);
              }
          }

          await userLevels.setMultiplierLevels(userLevelsArray, multipliers, feeDiscounts);
          await assertRevert(userLevels.setMultiplierLevels(userLevelsArray, multipliers, feeDiscounts, { from: user2 }));
          await assertRevert(userLevels.setMultiplierLevels([1,2,3], [1,2], [3,4]));
        ...
        it("1.2 Positions After increasing user levels", async () => {
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user3, marketId, 1))/1, 236815);
            // await cyclicMarkets.setNextOptionPrice(18);
            // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 2)) / 1, 18);
            // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[1] / 1, 18);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 98*1e8)/1, 0.32775*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user1, marketId, 2))/1, 313958);
            // await cyclicMarkets.setNextOptionPrice(27);
            // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 3)) / 1, 27);
            // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[2] / 1, 27);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 98*1e8)/1, 0.24283*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user4, marketId, 3))/1, 605361);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 392*1e8)/1, 0.53349*1e5);
            // await allMarkets.depositAndPlacePrediction(toWei(400), marketId, plotusToken.address, to8Power("400"), 2, { from: user2 });
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(400), marketId, plotusToken.address, to8Power("400"), 2);
            await signAndExecuteMetaTx(
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user2, marketId, 2))/1, 808262);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 1960000000)/1, 0.20095*1e5);
            // await allMarkets.depositAndPlacePrediction(toWei(1000), marketId, plotusToken.address, to8Power("1000"), 3, { from: user5 });
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(20), marketId, plotusToken.address, to8Power("20"), 3);
            await signAndExecuteMetaTx(
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user5, marketId, 3))/1, 208809);
        ...
        it("Should allow tx if sent by authorized relayer and time greater than expire time and less than settle time", async () => {
          await increaseTime(4 * 60 * 60 + 1);
          let marketData = await allMarkets.getMarketData(marketId);
          let expireTime = marketData[3];
          assert.isAbove(await latestTime(), expireTime/1);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 200000*0.98*1e8)/1, 0.98*1e5);
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(200000), marketId, plotusToken.address, to8Power("200000"), 1);
          ...
          await allMarkets.toggleAuthorizedRelayer(user0, true);
          await signAndExecuteMetaTx(
              privateKeyList[7],
              user6,
              functionSignature,
        ...
        it("1.3 Positions After increasing user levels, should not give multiplier for market creator's normal prediction", async () => {
          ...
          // console.log( //     predictionPointsBeforeUser1, //     predictionPointsBeforeUser2, //     predictionPointsBeforeUser3, //     predictionPointsBeforeUser4, //     predictionPointsBeforeUser5 // );
          const expectedPredictionPoints = [189452/1e5];
          const predictionPointArray = [
            predictionPoints/1
          ];
          for (let i = 0; i < predictionPointArray.length; i++) {
              assert.equal(expectedPredictionPoints[i], (predictionPointArray[i]).toFixed(5));
          }
...
// _optionLength, _predictionTime, _optionRangePerc, _marketStartTime, _marketSettleTime, _marketCooldownTime, _minTimePassed, _initialLiquidity)
      ...
      it("Create market", async () => {
        ...
        marketId++;
        assert.equal((await allMarkets.getUserPredictionPoints(userMarketCreator, marketId, 1))/1, 4320000);
        assert.equal((await allMarkets.getUserPredictionPoints(userMarketCreator, marketId, 2))/1, 4320000);

        assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 0.52113*1e5);
        assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 1960000000)/1, 0.52113*1e5);
      });
      ...
      it("1.1 Positions", async () => {
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user3, marketId, 2))/1, 37610);
          // await cyclicMarkets.setNextOptionPrice(18);
          // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 2)) / 1, 18);
          // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[1] / 1, 18);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 2415700000)/1, 52363);
          // await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power("100"), 2, { from: user1 });
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user1, marketId, 2))/1, 46133);
          // await cyclicMarkets.setNextOptionPrice(27);
          // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 3)) / 1, 27);
          // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[2] / 1, 27);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 51611);
          // await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power("100"), 3, { from: user4 });
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user4, marketId, 1))/1, 37976);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 51836);
          // await allMarkets.depositAndPlacePrediction(toWei(400), marketId, plotusToken.address, to8Power("400"), 2, { from: user2 });
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user2, marketId, 1))/1, 37811);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 2726360000)/1, 52205);
          // await allMarkets.depositAndPlacePrediction(toWei(1000), marketId, plotusToken.address, to8Power("1000"), 3, { from: user5 });
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user5, marketId, 2))/1, 52224);

          await cyclicMarkets.updateUintParameters(toHex("MAXP"), toWei(500000));
          
          // await allMarkets.depositAndPlacePrediction(toWei(190000), marketId, plotusToken.address, to8Power("190000"), 1, { from: user6 });
          
        await increaseTime(15 * 60 + 1);
          let marketData = await allMarkets.getMarketData(marketId);
          expireTime = marketData[3];
          assert.isAbove(await latestTime(), expireTime/1);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 37240000000)/1, 53621);
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(200000), marketId, plotusToken.address, to8Power("200000"), 1);
        //   await assertRevert(signAndExecuteMetaTx(
        //     privateKeyList[7],
        //     user6,
        ...
        //   await allMarkets.depositAndPlacePrediction(toWei(190000), marketId, plotusToken.address, to8Power("190000"), 1, { from: user6 });
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user6, marketId, 1))/1, 694504);
