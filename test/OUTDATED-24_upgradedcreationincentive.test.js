...
//         await assertRevert(allMarkets.createMarket(0, 0)); // Multiple same kind of markets can't exist at same time
...
//         await marketConfig.setNextOptionPrice(10);
//         await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power(100), 3, { from: user7 });
...
//             (oldBalance / 1e18 + (rewardPoolPlot * rewardPoolSharePerc) / 10000).toFixed(2)
//         );
//     });

//     it("Scenario 2: Should be able to get reward pool share of market", async function() {
//         marketId++;
//         await plotusToken.transfer(user2, toWei(25000));
//         await plotusToken.approve(tokenController.address, toWei(10000000), { from: user2 });
//         await tokenController.lock("0x534d", toWei(25000), 86400 * 30, { from: user2 });
//         await increaseTime(4 * 3600);
//         let tx = await allMarkets.createMarket(0, 0, { from: user2 });
...
//         await increaseTime(4 * 3600);
//         let tx = await allMarkets.createMarket(0, 0, { from: user10 });

//         await plotusToken.transfer(user7, toWei(1000));
//         await plotusToken.transfer(user7, toWei(5000));
//         await plotusToken.approve(allMarkets.address, toWei(10000000), { from: user7 });
...
//     it("Raise Dispute and pass: Scenario 2: Should be able to get reward pool share of market", async function() {
//         marketId++;
//         await increaseTime(86400*30);
//         await plotusToken.transfer(user10, toWei(25000));
//         await plotusToken.approve(tokenController.address, toWei(10000000), { from: user10 });
//         await plotusToken.approve(allMarkets.address, toWei(10000000), { from: user10 });
