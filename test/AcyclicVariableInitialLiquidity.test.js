...
contract("Rewards-Market", async function(users) {
	describe("Scenario1", async () => {
		...
		it("Scenario 1: Few user wins", async () => {
			...
			let betpoints = [0,5444.44444, 21777.77777, 11433.33333, 4464.44444, 54444.44444, 76222.22222, 10888.88888, 1814.81481, 10888.88888, 8166.66666, 5555.55555, 63888.88888];
			...
			assert.equal(218640000,Math.round(marketCreatorReward/1e11));

			// let creationReward = 14.3999;
			let marketCreatoFee = 21.864;
			...
			let userRewardPlot = [0,0,0,0,0,1933.710661,2707.194925,0,0,0,0];
			for(i=1;i<11;i++)
			{
			...
			assert.equal(await acyclicMarkets.rewardPoolShareForMarketCreator(users[11]),190.117*1e18);		
			...
			assert.equal(Math.round((plotBalAfterCreator-plotBalBeforeCreator)/1e11),1901170000);
...
contract("Rewards-Market Raise dispute and pass the proposal ", async function(users) {
	describe("Scenario1", async () => {
		...
		it("Scenario 1: Few user wins", async () => {
			...
			let userRewardPlot = [0, 0, 0, 0, 301.831931, 0, 0, 0, 122.695907, 736.175441, 0];
			for(i=1;i<11;i++)
			{
				let reward = await allMarkets.getReturn(users[i],7);
				assert.equal(Math.round(reward/1e2),userRewardPlot[i]*1e6);
			...
			assert.equal(Math.round((plotBalAfterCreator-plotBalBeforeCreator)/1e11),1951040000);
