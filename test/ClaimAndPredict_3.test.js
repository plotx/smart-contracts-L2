...
contract("Rewards-Market", async function(users) {
	...
	describe("Scenario1", async () => {
		it("0.0", async () => {
            ...
            _weth = await SampleERC.at(await claimAndPredict.nativeCurrencyAddress());
		...
		it("Scenario 1: Few user wins", async () => {
			...
			await claimAndPredict.whitelistTokenForSwap(await router.WETH());
			await assertRevert(claimAndPredict.whitelistTokenForSwap(nullAddress));
			await assertRevert(claimAndPredict.whitelistTokenForSwap(await router.WETH()));
			for(i=1; i<11;i++){
				await claimAndPredict.whitelistTokenForSwap(externalToken.address);
				if(i>1) {
                ...
                await externalToken.approve(claimAndPredict.address, _inputAmount, {from:users[i]});
                await externalToken.transfer(users[i], _inputAmount);
				await cyclicMarkets.setNextOptionPrice(options[i]*9);
                let spPlotBalanceBefore = await plotusToken.balanceOf(claimAndPredict.address); 
                let spTokenBalanceBefore = await externalToken.balanceOf(claimAndPredict.address); 
                // await spInstance.swapAndPlacePrediction([externalToken.address, plotusToken.address], _inputAmount, users[i], 7, options[i], 0)
			    let functionSignature;
				if(i == 2) {
                    _inputAmount = toWei((predictionVal[i]-20)*plotTokenPrice);
                    let signData = await getSignedClaimMessage(privateKeyList[0], users[i], 0, toWei(20), 1, 0, claimAndPredict.address);
                    let claimDataJson = {
                        user: users[i],
                        userClaimNonce: 0,
                        strategyId: 1,
                        claimAmount: toWei(20),
                        totalClaimed: 0,
                        v: signData.v,
                        r: signData.r,
                        s: signData.s
					...
					await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson, [await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 20*1e8, 1, {from:users[i], value:_inputAmount*2}));
					await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson, [await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 0, toWei(1000), {from:users[i], value:_inputAmount}));
					await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson, [await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 0, predictionVal[i], {from:users[1], value:_inputAmount}));
					await claimAndPredict.deWhitelistTokenForSwap(await router.WETH());
					await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson ,[await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 20*1e8, predictionVal[i]-20, {from:users[i], value:_inputAmount}));
					await claimAndPredict.whitelistTokenForSwap(await router.WETH());

					await claimAndPredict.claimSwapAndPredict(claimDataJson ,[await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 20*1e8, predictionVal[i]-20, {from:users[i], value:_inputAmount});
                } else {
                    let signData = await getSignedClaimMessage(privateKeyList[0], users[i], 0, toWei(20), 1, 0, claimAndPredict.address);
                    let claimDataJson = {
                        user: users[i],
                    ...
                    _inputAmount = toWei((predictionVal[i]-20)*plotTokenPrice);
                    await externalToken.transfer(users[i], toWei(100000));
                    await externalToken.approve(claimAndPredict.address, toWei(10000000), {from:users[i]});
					if(i == 3) {
						//Predict with WETH
						await _weth.approve(claimAndPredict.address, _inputAmount, {from:users[i]});
                        ...
                        functionSignature = await claimAndPredict.contract.methods.claimSwapAndPredict(claimDataJson ,[await router.WETH(), plotusToken.address], _inputAmount, 7, options[i], 20*1e8, predictionVal[i]).encodeABI();
                        // functionSignature = encode3("claimSwapAndPredict(address[],uint256,address,uint256,uint256,uint64,uint256)", [await router.WETH(), plotusToken.address], _inputAmount, users[i], 7, options[i], 20*1e8, predictionVal[i]);
					} else {
						await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson, [externalToken.address, externalToken.address], _inputAmount, 7, options[i], 20*1e8, predictionVal[i], {from:users[i]}));
                        await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson ,[externalToken.address, plotusToken.address], _inputAmount, 7, options[i], 20*1e8, 1));
                        await assertRevert(claimAndPredict.claimSwapAndPredict(claimDataJson ,[externalToken.address, plotusToken.address], _inputAmount, 7, options[i], 20*1e8, 1, {from:users[i], value:_inputAmount}));
                        functionSignature = await claimAndPredict.contract.methods.claimSwapAndPredict(claimDataJson ,[externalToken.address, plotusToken.address], _inputAmount, 7, options[i], 20*1e8, predictionVal[i]).encodeABI();
						// functionSignature = encode3("claimSwapAndPredict(address[],uint256,address,uint256,uint256,uint64,uint256)", [externalToken.address, plotusToken.address], _inputAmount, users[i], 7, options[i], 20*1e8, predictionVal[i]);
					}  
                ...
                // let spPlotBalanceAfter = await plotusToken.balanceOf(claimAndPredict.address); 
                // let spTokenBalanceAfter = await externalToken.balanceOf(claimAndPredict.address); 
				...
				await claimAndPredict.deWhitelistTokenForSwap(externalToken.address);
			}
			await assertRevert(claimAndPredict.deWhitelistTokenForSwap(externalToken.address));
