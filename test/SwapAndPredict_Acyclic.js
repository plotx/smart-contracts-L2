...
contract("Rewards-Market", async function(users) {
	describe("Scenario1", async () => {
		...
		it("Scenario 1: Few user wins", async () => {
            ...
            //Subtract with 1.6 as for prediction at index 2 is sent directly and relayer will not get any rewards for that 
			assert.equal(Math.round((relayerBalAfter-relayerBalBefore)/1e15),11.532*1e3-1.6*1e3);
