...
describe("Option pricing V2 with Multiplier: 3 bucket market", () => {
    ...
    contract("AllMarkets", async function ([user0, user1, user2, user3, user4, user5, userMarketCreator, user6]) {
        ...
        it("Create market", async () => {
          ...
          assert.equal(optionPrices[0] / 1, 0.51944*1e5);
          assert.equal(optionPrices[1] / 1, 0.51944*1e5);
          assert.equal(optionPrices[2] / 1, 0.51944*1e5);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 98*1e8)/1, 0.51944*1e5);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 98*1e8)/1, 0.51944*1e5);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 98*1e8)/1, 0.51944*1e5);
        });
        ...
        it("1.1 Position without User levels", async () => {
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user3, marketId, 1))/1, 188664);
            // await cyclicMarkets.setNextOptionPrice(18);
            ...
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 98*1e8)/1, 0.32722*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user1, marketId, 2))/1, 299492);
            ...
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 98*1e8)/1, 0.24184*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user4, marketId, 3))/1, 405226);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 392*1e8)/1, 0.53415*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user2, marketId, 2))/1, 733876);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 1960000000)/1, 0.2005*1e5);
            // await allMarkets.depositAndPlacePrediction(toWei(1000), marketId, plotusToken.address, to8Power("1000"), 3, { from: user5 });
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(20), marketId, plotusToken.address, to8Power("20"), 3);
            await signAndExecuteMetaTx(
              privateKeyList[5],
              user5,
              functionSignature,
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user5, marketId, 3))/1, 97755);
        ...
        it("1.2 Positions After increasing user levels", async () => {
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user3, marketId, 1))/1, 235830);
            // await cyclicMarkets.setNextOptionPrice(18);
            // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 2)) / 1, 18);
            // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[1] / 1, 18);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 98*1e8)/1, 0.32722*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user1, marketId, 2))/1, 314466);
            // await cyclicMarkets.setNextOptionPrice(27);
            // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 3)) / 1, 27);
            // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[2] / 1, 27);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 98*1e8)/1, 0.24184*1e5);
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user4, marketId, 3))/1, 607839);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 392*1e8)/1, 0.53415*1e5);
            // await allMarkets.depositAndPlacePrediction(toWei(400), marketId, plotusToken.address, to8Power("400"), 2, { from: user2 });
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(400), marketId, plotusToken.address, to8Power("400"), 2);
            await signAndExecuteMetaTx(
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user2, marketId, 2))/1, 807263);
            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 3, 1960000000)/1, 0.2005*1e5);
            // await allMarkets.depositAndPlacePrediction(toWei(1000), marketId, plotusToken.address, to8Power("1000"), 3, { from: user5 });
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(20), marketId, plotusToken.address, to8Power("20"), 3);
            await signAndExecuteMetaTx(
            ...
            assert.equal((await allMarkets.getUserPredictionPoints(user5, marketId, 3))/1, 205285);

            assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 200000*0.98*1e8)/1, 0.98*1e5);
            functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(200000), marketId, plotusToken.address, to8Power("200000"), 1);
            await signAndExecuteMetaTx(
              privateKeyList[7],
              user6,
              functionSignature,
        ...
        it("1.3 Positions After increasing user levels, should not give multiplier for market creator's normal prediction", async () => {
          await increaseTime(4 * 60 * 60 + 1);
          ...
          // console.log( //     predictionPointsBeforeUser1, //     predictionPointsBeforeUser2, //     predictionPointsBeforeUser3, //     predictionPointsBeforeUser4, //     predictionPointsBeforeUser5 // );
          const expectedPredictionPoints = [188664/1e5];
          const predictionPointArray = [
            predictionPoints/1
          ];
          for (let i = 0; i < predictionPointArray.length; i++) {
...
describe("Option pricing V2 with Multiplier: 2 bucket market", () => {
  let masterInstance,
      plotusToken,
      allMarkets;
  let marketId = 1;
  ...
  contract("AllMarkets", async function ([user0, user1, user2, user3, user4, user5, userMarketCreator, user6]) {
      ...
      it("Upgrade contract", async () => {
          ...
          // await cyclicMarkets.setNextOptionPrice(18);
          // await cyclicMarkets.alterMarketType(0, 3, 100, 8 * 60 * 60, 60 * 60, 40 * 60, 100 * 1e8);
// _optionLength, _predictionTime, _optionRangePerc, _marketStartTime, _marketSettleTime, _marketCooldownTime, _minTimePassed, _initialLiquidity)
          await cyclicMarkets.newMarketType(2, 15*60 , 100, await latestTime(), 1 * 60 * 60, 5 * 60, 5 * 60, 4320 * 1e8);
      });

      it("Create market", async () => {
        await cyclicMarkets.createMarketWithOptionRanges(0, 3, [119500000000], { from: userMarketCreator });
        marketId++;
        assert.equal((await allMarkets.getUserPredictionPoints(userMarketCreator, marketId, 1))/1, 4233600);
        assert.equal((await allMarkets.getUserPredictionPoints(userMarketCreator, marketId, 2))/1, 4233600);

        assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 0.52115*1e5);
        assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 1960000000)/1, 0.52115*1e5);
      });
      ...
      it("1.1 Positions", async () => {
          ...
          // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 1)) / 1, 9);
          // await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power("100"), 1, { from: user3 });
          let functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(20), marketId, plotusToken.address, to8Power("20"), 2);
          await signAndExecuteMetaTx(
            privateKeyList[3],
            user3,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user3, marketId, 2))/1, 37609);
          // await cyclicMarkets.setNextOptionPrice(18);
          // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 2)) / 1, 18);
          // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[1] / 1, 18);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 2415700000)/1, 0.52371*1e5);
          // await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power("100"), 2, { from: user1 });
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(100), marketId, plotusToken.address, 2465000000, 2);
          await signAndExecuteMetaTx(
            privateKeyList[1],
            user1,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user1, marketId, 2))/1, 46126);
          // await cyclicMarkets.setNextOptionPrice(27);
          // assert.equal((await cyclicMarkets.getOptionPrice(marketId, 3)) / 1, 27);
          // assert.equal((await allMarkets.getMarketData(marketId))._optionPrice[2] / 1, 27);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 0.51603*1e5);
          // await allMarkets.depositAndPlacePrediction(toWei(100), marketId, plotusToken.address, to8Power("100"), 3, { from: user4 });
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(100), marketId, plotusToken.address, to8Power("20"), 1);
          await signAndExecuteMetaTx(
            privateKeyList[4],
            user4,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user4, marketId, 1))/1, 37982);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 1960000000)/1, 51833);
          // await allMarkets.depositAndPlacePrediction(toWei(400), marketId, plotusToken.address, to8Power("400"), 2, { from: user2 });
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(400), marketId, plotusToken.address, to8Power("20"), 1);
          await signAndExecuteMetaTx(
            privateKeyList[2],
            user2,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user2, marketId, 1))/1, 37813);
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 2, 2726360000)/1, 52209);
          // await allMarkets.depositAndPlacePrediction(toWei(1000), marketId, plotusToken.address, to8Power("1000"), 3, { from: user5 });
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(30), marketId, plotusToken.address, 2782000000, 2);
          await signAndExecuteMetaTx(
            privateKeyList[5],
            user5,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user5, marketId, 2))/1, 52220);

          await cyclicMarkets.updateUintParameters(toHex("MAXP"), toWei(500000));
          assert.equal(await cyclicMarkets.getOptionPriceWithStake(marketId, 1, 37240000000)/1, 53651);
          // await allMarkets.depositAndPlacePrediction(toWei(190000), marketId, plotusToken.address, to8Power("190000"), 1, { from: user6 });
          functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(190000), marketId, plotusToken.address, 38000000000, 1);
          await signAndExecuteMetaTx(
            privateKeyList[7],
            user6,
            functionSignature,
          ...
          assert.equal((await allMarkets.getUserPredictionPoints(user6, marketId, 1))/1, 694115);
