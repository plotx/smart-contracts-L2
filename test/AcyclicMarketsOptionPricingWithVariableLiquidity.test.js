...
const AllMarkets_5 = artifacts.require("AllPlotMarkets_5");
...
contract("Option Pricing", async function(users) {
	describe("Scenario1", async () => {
		it("0.0", async () => {
			...
			await acyclicMarkets.setNextOptionPrice(0);
		...
		it("Issue: Should return option price = 0 if staked on option is less than totalstake/1e5", async () => {

			await acyclicMarkets.createMarket("Question1", [100,400], [timeNow/1+4*3600,timeNow/1+8*3600,3600],toHex("NFT"),toHex("PLOT"), 120*10**8,{from:users[11],gasPrice:500000});
			await acyclicMarkets.updateUintParameters(toHex("MAXP"),toWei(5000000));
			let predictionval = 4500000;
			await plotusToken.mint(users[0],toHex(4500000));
			await plotusToken.approve(allMarkets.address,toWei(5000000));
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(predictionval), 7, plotusToken.address, (predictionval*1e8).toFixed(0), 1);
			...
			assert.equal(optionPrices[2], 0); 


		});


		it("Should fix the issue after Upgrade contract, option price = 0.01", async () => {
			...
			assert.equal(optionPrices[2], 1000); 
		...
		it("Scenario 1: Option price for Market Creator", async () => {
			...
			await acyclicMarkets.createMarketWithVariableLiquidity("Question1", [2,3], [timeNow/1+4*3600,timeNow/1+8*3600,3600],toHex("IPL"),toHex("Cricket-IPL"), [100*10**8,100*10**8,100*10**8],{from:users[11],gasPrice:500000});
			assert.equal(await allMarkets.getUserPredictionPoints(users[11],8,1),Math.floor(100*1e8/33333)); 
			assert.equal(await allMarkets.getUserPredictionPoints(users[11],8,2),Math.floor(100*1e8/33333)); 
			assert.equal(await allMarkets.getUserPredictionPoints(users[11],8,3),Math.floor(100*1e8/33333)); 
		...
		it("Scenario 2: Option price when total stake < minstake", async () => {
			let functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(9900), 8, plotusToken.address, 9900*1e8, 1);
				await signAndExecuteMetaTx(
			      privateKeyList[0],
			      users[0],
			      functionSignature,
			...
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(8900), 8, plotusToken.address, 8900*1e8, 2);
				await signAndExecuteMetaTx(
			      privateKeyList[0],
			      users[0],
			      functionSignature,
			...
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(1300), 8, plotusToken.address, 1300*1e8, 3);
				await signAndExecuteMetaTx(
			      privateKeyList[0],
			      users[0],
			      functionSignature,
			...
			let optionPriceData3 = await allMarkets.getMarketOptionPricingParams(8,3);
			assert.equal(optionPriceData1[0][0], 9802*1e8);
			assert.equal(optionPriceData2[0][0], 8822*1e8);
			assert.equal(optionPriceData3[0][0], 1374*1e8);
			assert.equal(optionPriceData1[0][1], 19998*1e8);

			let optionPrices = await acyclicMarkets.getAllOptionPrices(8);
		...
		it("Scenario 3: Option price when total stake > minstake", async () => {
			...
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", toWei(3592), 8, plotusToken.address, 3592*1e8, 3);
				await signAndExecuteMetaTx(
			      privateKeyList[0],
			      users[0],
			      functionSignature,
			...
			let optionPriceData2 = await allMarkets.getMarketOptionPricingParams(8,2);
			let optionPriceData3 = await allMarkets.getMarketOptionPricingParams(8,3);
			assert.equal(optionPriceData1[0][1], 23518.16*1e8);
			assert.equal(optionPriceData1[0][0], 9802*1e8);
			assert.equal(optionPriceData2[0][0], 8822*1e8);
			assert.equal(optionPriceData3[0][0], 4894.16*1e8);

			let optionPrices = await acyclicMarkets.getAllOptionPrices(8);

			assert.equal(optionPrices[0], 41678); // 9802/23518.16
			assert.equal(optionPrices[1], 37511); // 8822/23518.16
			assert.equal(optionPrices[2], 20810); // 4894.16/23518.16
		});
