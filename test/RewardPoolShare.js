...
contract("Markets", async function(users) {
    ...
    it("Create markets for 4 weeks, upgrade contract after three weeks and should work properly", async () => {
        let z = 7;
        let allMarketsV3Impl = await AllMarkets_V3.new();
        ...
        for(let i = 0;i<4; i++) {
            ...
            if(i == 3) {
                let plotBalance = await allMarkets.getUserUnusedBalance(users[0]);
                await allMarkets.withdraw(plotBalance[0].iadd(plotBalance[1]), 1000)
            }
            for(j = 0;j<7;j++) {
                ...
                for(k = 0;k<3;k++) {
                    ...
                    await increaseTime(2*4*60*60 + 1);
                }
            }
        }
        ...
        let tx;
        try {
			if((plotBalance[0].iadd(plotBalance[1]))/1 > 0)
