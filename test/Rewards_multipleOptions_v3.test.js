...
const AllMarkets_V3 = artifacts.require("MockAllMarkets_3");
...
describe("Bets Multiple options sheet", () => {
    contract("AllMarket", async function ([user1, user2, user3, user4, user5, user6, userMarketCreator]) {
        ...
        it("3.1 Scenario 1: player purchase 2 position in same option, in same currency and wins", async () => {
            ...
            await assertRevert(allMarkets.postResultMock(0, marketId));
            ...
            await increaseTime(8 * 60 * 60);
            await allMarkets.emitMarketSettledEvent(marketId);
        ...
        it("3.6. Scenario 6,7 and 8", async () => {
            ...
            await increaseTime(8 * 60 * 60);
            await allMarkets.emitMarketSettledEvent(scenario7MarketId);
            await allMarkets.emitMarketSettledEvent(scenario8MarketId);
            ...
            await allMarkets.emitMarketSettledEvent(scenario6MarketId);
