...
contract CyclicMarkets is IAuth, NativeMetaTransaction {
    ...
    */
    function addInitialMarketTypesAndStart(uint32 _marketStartTime, address _ethFeed, address _btcFeed) external onlyAuthorizedUsers {
      ...
      totalOptions = 3;
      stakingFactorMinStake = uint(20000).mul(10**8); // need to update (in usd)
      ...
      // update initialLiquidity (in usd)
    ...
    */
    function createMarket(uint32 _marketCurrencyIndex,uint32 _marketTypeIndex, uint80 _roundId, uint _initialLiquidityAssetIndex) public {
    ...
    */
    function calculatePredictionPoints(uint _marketId, uint256 _prediction, address _user, bool _multiplierApplied, uint _predictionStake, address _asset) internal view returns(uint64 predictionPoints, bool isMultiplierApplied) {
    ...
    */
    ...
    function getOptionPrice(uint _marketId, uint256 _prediction) public view returns(uint64) {
      ...
      uint optionPrice; 
      uint256 totalStaked = _optionPricingParams[1]; // fetch usd equivalet stake for all asset
